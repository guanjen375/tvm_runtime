...
extern std::unordered_map<std::string,kernelhndl> kernelmap;
namespace tvm {
namespace runtime {
namespace details {
...
void GraphRuntime::Run() {
  ...
  for (i = 0; i < op_execs_.size(); ++i) 
  {
    ...
    if (op_execs_[i]) 
    {
        ...
        time = (double)total_cycles/5000000;
        std::cout << "Running:" << ((float)(i*100))/op_execs_.size() << "%";
        std::cout << "( Total Time: " << (int)(time/60) << " min ";
        std::cout << ((int)(time))%60 << " sec ," ;
        std::cout << " Name:" << nodes_[i].name << " )\n";
    }
  }
}
/*!
...
NDArray GraphRuntime::GetNode(int index) const{
  return data_entry_[index];
}
/*!
...
std::pair<std::function<void()>, std::shared_ptr<GraphRuntime::OpArgs> > GraphRuntime::CreateTVMOp(
  ...
  if (param.func_name == "__nop") {
    ...
    auto fexec = [arg_ptr]() {
      DLTensor* from;
      DLTensor* to;
      int length = arg_ptr->arg_values.size() ;
      for(int i=0;i<length/2;i=i+1)
      {
        from = static_cast<DLTensor*>(arg_ptr->arg_values[i].v_handle);
        to = static_cast<DLTensor*>(arg_ptr->arg_values[i+length/2].v_handle);
  ...
  std::string name = param.func_name + "_kernel0" ;
...
PackedFunc GraphRuntime::GetFunction(
  ...
  } else if (name == "debug_node"){
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
    *rv = this->GetNode(args[0]);
    });
  } 
