...
//extern "C" kernelfunc _fused_nn_dense_add_nn_relu_kernel0_workgroup_fast;
...
int64_t ArrayGetHashValue_int(const DLTensor* handle, size_t arr_size,std::string type){
  int64_t result = 0;
  if(type=="int8")
    for(int i=0;i<arr_size;i++)
      result += ((int8_t *)(handle->data))[i];
  if(type=="uint8")
    for(int i=0;i<arr_size;i++)
      result += ((uint8_t *)(handle->data))[i];
  if(type=="int32")
    for(int i=0;i<arr_size/4;i++)
      result += ((int *)(handle->data))[i];
  if(type=="int64")
    for(int i=0;i<arr_size/8;i++)
      result += ((int64_t *)(handle->data))[i];
  return result;
}

double ArrayGetHashValue_float(const DLTensor* handle, size_t arr_size){
  double result = 0;
  for(int i=0;i<arr_size;i++)
    result += ((float *)(handle->data))[i];
  return result;
}
...
int64_t NDArray::GetHashValue_int(size_t arr_size,std::string type) const {
  int64_t result = ArrayGetHashValue_int(&get_mutable()->dl_tensor,arr_size,type);
  return result;
}

double NDArray::GetHashValue_float(size_t arr_size) const {
  double result = ArrayGetHashValue_float(&get_mutable()->dl_tensor,arr_size);
  return result;
}
